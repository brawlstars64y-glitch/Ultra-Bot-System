...
http.createServer((req, res) => {
    ...
    res.end("نظام MaxBlack Ultra المتكامل يعمل 🛡️");
...
bot.start((ctx) => {
    ctx.replyWithMarkdown(`*• مرحباً بك يا بطل في نظام MaxBlack المتكامل* 🔮\n*تم تفعيل حماية الانهيار و Anti-AFK تلقائياً* 🛡️`, mainUI);
});

// ⚙️ تفعيل زر إعدادات النظام
bot.action('settings', (ctx) => {
    ctx.editMessageText(`⚙️ *إعدادات النظام (MaxBlack):*\n\n• *حماية الانهيار:* مفعلة ✅\n• *نظام Anti-AFK:* مفعل ✅\n• *الإصدار:* 1.21.130 🛠️\n\n_هذه الإعدادات يتم ضبطها تلقائياً لضمان أفضل أداء لبوتك._`, {
        parse_mode: 'Markdown',
...
// ❓ تفعيل زر المساعدة
bot.action('help', (ctx) => {
    ctx.editMessageText(`❓ *دليل المساعدة يا بطل:*\n\n1️⃣ اضغط على "إضافة سيرفر" وأرسل الآي بي ثم البورت.\n2️⃣ اذهب إلى "سيرفراتي" واضغط "تشغيل البوت".\n3️⃣ سيقوم البوت بالدخول وإرسال رسالة ترحيب والبقاء نشطاً.\n\n⚠️ *ملاحظة:* تأكد أن السيرفر يعمل قبل التشغيل.`, {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([[Markup.button.callback('🔙 رجوع للقائمة', 'home')]])
    });
});

// 🛠️ نظام إضافة السيرفر (خطوة بخطوة)
...
bot.on('text', async (ctx) => {
    ...
    if (ctx.session?.step === 'get_host') {
        ...
        ctx.session = null;
        ctx.reply('✅ *تم حفظ البيانات! السيرفر جاهز الآن.*', mainUI);
    }
});
...
bot.action(/^manage_(\d+)$/, (ctx) => {
    ...
    ctx.editMessageText(`*لوحة تحكم السيرفر* 📊\n--------------------------\n🌐 \`${s.host}:${s.port}\`\n📊 الحالة: ${status}`, {
        parse_mode: 'Markdown',
...
// ▶️ المحرك المتكامل (Anti-AFK + Crash Safe + 1.21.130)
bot.action(/^toggle_(\d+)$/, async (ctx) => {
    ...
    try {
        ctx.answerCbQuery("⏳ جاري الاقتحام...");
        activeClients[userId] = bedrock.createClient({
            host: s.host, port: parseInt(s.port), username: s.bot_name,
            offline: true, version: '1.21.130', skipPing: true, connectTimeout: 30000
        });

        activeClients[userId].on('spawn', () => {
            ctx.reply(`✅ *أبشر يا بطل! البوت دخل السيرفر الآن.*`);
            activeClients[userId].queue('text', { type: 'chat', needs_translation: false, source_name: s.bot_name, xuid: '', platform_chat_id: '', message: '🛡️ MaxBlack System Active' });
            
            // 🔄 نظام Anti-AFK
            afkIntervals[userId] = setInterval(() => {
                if (activeClients[userId]) {
                    activeClients[userId].queue('text', { type: 'chat', needs_translation: false, source_name: s.bot_name, xuid: '', platform_chat_id: '', message: '💎 Stay Active Pulse' });
                }
            }, 45000);
        });
...
bot.launch();
console.log('🚀 نظام MaxBlack Ultra المتكامل شغال الآن!');
