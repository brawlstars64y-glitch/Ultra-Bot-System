...
// لضمان استقرار Railway
http.createServer((req, res) => {
  res.writeHead(200)
  res.end('MAX BLACK BOT IS ACTIVE')
...
bot.start(async ctx => {
  if (!(await checkSub(ctx))) {
    return ctx.reply('⚠️ اشترك بالقنوات أولاً لتعمل اللوحة:', Markup.inlineKeyboard([
      [Markup.button.callback('✅ تم الاشتراك', 'CHECK')]
    ]))
  }
  ctx.reply('🎮 أهلاً بك يا بطل، اختر خياراً:', menu())
})

bot.action('CHECK', async ctx => {
  if (await checkSub(ctx)) ctx.editMessageText('✅ تم التحقق، أهلاً بك!', menu())
  else ctx.answerCbQuery('❌ اشترك في كل القنوات أولاً!', { show_alert: true })
})
...
bot.on('text', ctx => {
  ...
  if (!ctx.message.text.includes(':')) return ctx.reply('❌ الصيغة خاطئة، مثال: 123.45.67:19132')
...
bot.action('LIST', ctx => {
  ...
  if (!list || list.length === 0) return ctx.reply('📭 لا يوجد سيرفرات.', menu())
  const btns = list.map((s, i) => [Markup.button.callback(`📍 ${s.host}:${s.port}`, `SRV_${i}`)])
  btns.push([Markup.button.callback('⬅️ رجوع', 'BACK')])
  ctx.reply('📂 سيرفراتك المضافة:', Markup.inlineKeyboard(btns))
})
...
bot.action(/^TOGGLE_(\d+)$/, ctx => {
  ...
  if (clients[uid]) { clients[uid].close(); delete clients[uid]; return ctx.reply('⏹ تم إخراج البوت.') }

  ctx.reply('⏳ جاري الدخول (1.20 - 1.21.132)...')
  try {
...
bot.launch().then(() => console.log('✅ BOT IS GREEN AND READY'))
