...
const SUPPORTED_VERSIONS = [
  ...
  '1.21.130', '1.21.120', '1.21.110', '1.21.100', '1.21.90', '1.21.80', '1.21.70', '1.21.60', '1.21.50', '1.21.40', '1.21.30', '1.21.20', '1.21.10', '1.21.0',
...
function versionMenu() {
  ...
  rows.push([
    ...
    Markup.button.callback('📜 المزيد', 'show_all_versions')
  ])
...
bot.action('select_version', async (ctx) => {
  ...
  const currentVersion = server.version || 'اكتشاف تلقائي'
  
  ctx.reply(
    `🎮 **اختر إصدار Minecraft**\n\n` +
    `السيرفر الحالي: ${server.name}\n` +
...
bot.action('show_all_versions', async (ctx) => {
  ...
  // عرض أول 30 إصدار فقط
  const displayVersions = SUPPORTED_VERSIONS.slice(0, 30)
  const versionList = displayVersions.map((v, i) => `${i+1}. ${v}`).join('\n')
  
  ctx.reply(
    `📜 **الإصدارات المدعومة**\n\n${versionList}\n\n` +
    `🔄 **الإجمالي:** ${SUPPORTED_VERSIONS.length} إصدار\n` +
    `🎮 **الأحدث:** ${SUPPORTED_VERSIONS[0]}\n\n` +
    `اختر إصداراً من القائمة السابقة`,
    {
      parse_mode: 'Markdown'
    }
  )
    ...
    case 'bot_username':
      ...
      try {
        const newServer = {
          ...
          version: null, // ⭐ الإصلاح: null بدلاً من false
        ...
        ctx.reply(
          ...
          `💡 **ملاحظة:** البوت سيحاول الاتصال بإصدار 1.21.130 تلقائياً\n` +
          `يمكنك تغيير الإصدار من 🔧 إعدادات متقدمة`,
          {
            parse_mode: 'Markdown',
            reply_markup: mainMenu().reply_markup
          }
...
bot.action(/select_(\d+)/, requireSubscription, async (ctx) => {
  ...
  const versionInfo = selectedServer.version ? 
    ...
    `🔄 **الإصدار:** اكتشاف تلقائي (سيحاول 1.21.130)`
...
bot.action(/version_(.+)/, async (ctx) => {
  ...
  if (serverIndex !== -1) {
    if (version === 'auto') {
      ctx.session.servers[serverIndex].version = null // ⭐ الإصلاح: null بدلاً من false
      ctx.session.currentServer.version = null
    } else {
...
bot.action('detect_version', async (ctx) => {
  ...
  const server = ctx.session.currentServer
  ctx.reply(`🔄 جاري محاولة الاتصال لاكتشاف إصدار ${server.name}...`)
  
  try {
    // محاولة الاتصال مع إصدارات شائعة
    const commonVersions = ['1.21.130', '1.20.80', '1.19.84', '1.18.33']
    ...
    for (const testVersion of commonVersions) {
      try {
        ...
        console.log(`🔍 اختبار الإصدار: ${testVersion}`)
        const testClient = bedrock.createClient(options)
        
        await new Promise((resolve, reject) => {
          testClient.on('connect_allowed', () => {
        ...
        console.log(`❌ فشل مع ${testVersion}:`, err.message)
      }
    }
    ...
    if (detectedVersion) {
      ...
      ctx.reply(
        `⚠️ **لم يتمكن من اكتشاف الإصدار**\n\n` +
        `جرب:\n` +
        `1. اختر إصداراً يدوياً من القائمة\n` +
        `2. أو استخدم إصدار 1.21.130 (الأحدث)`,
        {
          parse_mode: 'Markdown',
          reply_markup: mainMenu().reply_markup
        }
...
bot.action('list_versions', async (ctx) => {
  ...
  ctx.reply(
    ...
    `🔄 **الإجمالي:** ${SUPPORTED_VERSIONS.length} إصدار\n` +
    `🎯 **الأحدث:** ${SUPPORTED_VERSIONS[0]}\n` +
    `💡 **نصيحة:** استخدم إصدار 1.21.130 للسيرفرات الحديثة`,
    {
      parse_mode: 'Markdown',
      reply_markup: mainMenu().reply_markup
    }
...
/* ⭐ الإصلاح: دخول للسيرفر مع إصلاح مشكلة الإصدار */
bot.action('connect', requireSubscription, async (ctx) => {
  ...
  const versionText = server.version ? server.version : 'اكتشاف تلقائي (سيحاول 1.21.130)'
  ...
  try {
    ...
    // ⭐⭐ الإصلاح الحاسم: تحديد إصدار افتراضي إذا لم يكن محدداً
    if (server.version) {
      options.version = server.version
      console.log(`🎮 استخدام الإصدار المحدد: ${server.version}`)
    } else {
      // ⭐⭐ استخدام إصدار افتراضي (1.21.130) بدلاً من false
      options.version = '1.21.130' // ⭐ الإصلاح: إصدار محدد بدلاً من false
      console.log('🔄 استخدام الإصدار الافتراضي: 1.21.130')
    }
    ...
    const attemptConnection = async () => {
      try {
        ...
        // إذا فشل مع 1.21.130، جرب إصداراً أقدم
        if (retryCount <= maxRetries && !server.version) {
          const fallbackVersions = ['1.20.80', '1.19.84']
          const fallbackVersion = fallbackVersions[retryCount - 1]
          
          ctx.reply(`⚠️ إعادة المحاولة مع إصدار ${fallbackVersion}...`)
          console.log(`🔄 إعادة المحاولة مع إصدار: ${fallbackVersion}`)
          
          options.version = fallbackVersion
    ...
    client = await attemptConnection()
    ...
    client.on('spawn', () => {
      const connectedVersion = client.version || options.version
    ...
    client.on('error', (err) => {
      ...
      if (err.message.includes('version') || err.message.includes('unsupported')) {
        ...
        if (options.version === '1.21.130') {
          errorMessage += `4. جرب إصدارات مثل: 1.20.80, 1.19.84\n`
        }
      }
    ...
    if (error.message.includes('version')) {
      ...
      errorMessage += `2. اضغط 🎮 اختيار إصدار\n`
      errorMessage += `3. اختر إصدار 1.20.80 أو 1.19.84\n`
...
bot.action('status', requireSubscription, async (ctx) => {
  ...
  if (ctx.session.currentServer) {
    ...
    const versionText = server.version ? 
      `🎮 **الإصدار:** ${server.version}` : 
      `🔄 **الإصدار:** اكتشاف تلقائي (سيحاول 1.21.130)`
    
    statusMessage += `${versionText}\n\n`
...
bot.launch({
  ...
  console.log(`🎯 الإصدار الافتراضي: 1.21.130`)
  console.log('📢 الاشتراك الإجباري مفعل')
  console.log('🔧 تم إصلاح مشكلة الإصدار غير المدعوم')
  console.log('===========================')
})
...
bot.command('setversion', requireSubscription, async (ctx) => {
  if (!ctx.session.currentServer) {
    return ctx.reply('⚠️ اختر سيرفراً أولاً.')
  }
  ...
  if (args.length < 2) {
    return ctx.reply('❌ استخدم: `/setversion 1.21.130`')
  }
  
  const version = args[1]
  if (!SUPPORTED_VERSIONS.includes(version)) {
    return ctx.reply(`❌ الإصدار ${version} غير مدعوم.\nاستخدم /versions لعرض الإصدارات المدعومة`)
  }
  ...
  if (serverIndex !== -1) {
    ...
    ctx.reply(`✅ تم تعيين إصدار ${server.name} إلى ${version}`)
  }
})
...
bot.command('versions', async (ctx) => {
  ...
  ctx.reply(
    ...
    `🎯 **الأحدث:** ${SUPPORTED_VERSIONS[0]}\n\n` +
    `💡 **نصيحة:**\n` +
    `• للسيرفرات الحديثة استخدم 1.21.130\n` +
    `• للسيرفرات القديمة جرب 1.20.80 أو 1.19.84\n` +
    `• لتغيير إصدار: /setversion [الإصدار]`,
    {
      parse_mode: 'Markdown'
    }
  )
...
bot.command('test', requireSubscription, async (ctx) => {
  const testInfo = {
    servers: ctx.session.servers ? ctx.session.servers.length : 0,
    currentServer: ctx.session.currentServer ? {
      name: ctx.session.currentServer.name,
      version: ctx.session.currentServer.version || 'اكتشاف تلقائي'
    } : 'لا يوجد',
  ...
  ctx.reply(
    ...
    `🎯 السيرفر المختار: ${testInfo.currentServer.name || 'لا يوجد'}\n` +
    `🎮 إصدار السيرفر: ${testInfo.currentServer.version || 'لا يوجد'}\n` +
    `🔗 اتصالات نشطة: ${testInfo.connections}\n` +
    `📋 إصدارات مدعومة: ${testInfo.supportedVersions}\n` +
    `🎯 الإصدار الافتراضي: 1.21.130`,
    {
      parse_mode: 'Markdown'
    }
  )
